...
import { DataType, SegmentType } from './SqSyNewsByteTypes/enums';
import { ShowData, ShowChapterData, SegmentChapterData, MusicChapterData, AlternateData } from './SqSyNewsByteTypes/interfaces';
...
const sqsyCompanion = require('./SqSyNewsByteTypes/companion.json');
...
const SquareSymOps: SquareSymOpsType = {
  ...
  TagProcess: async (data, { inputfileObj, makeLog, doUpload, browser }) => {
    if (SquareSymOps.running) {
      econsole.error('SqSyNewsByte process already running.');
      return;
    }
    ...
    const outputfile: string = `NewsByte for ${data.airdate}`;
    ...
    const tagOperation = inputfileBuffer ? (async () => {
      ...
      if (!fs.existsSync('./output') || !fs.existsSync('./output/SqSyNewsByte')) fs.mkdirSync(BASE_OUTPUTPATH, { recursive: true });
  ...
  ReaperProcess: async (inputfile: string, reaperData: ReaperReader) => {
    ...
    if (!fs.existsSync('./showdata') || !fs.existsSync('./showData/SqSyNewsByte')) fs.mkdirSync(BASE_DATAPATH, { recursive: true });
  ...
  FindMusicPath: (data: ShowData, datestamp: string | any[]) => {
    let dirs = fs.readdirSync(`${BASE_DATAPATH}NB/`, { withFileTypes: true })
      .filter(i => i.isDirectory() && i.name.substr(0, datestamp.length) === `${datestamp}`)
      .map(i => i.name);
    if (dirs.length > 0) return `${BASE_DATAPATH}NB/${dirs[0]}/`;
  ...
  GenerateTags: (data: ShowData, { outputfile, musicPath }: GenerateTagsOptions) => {
    ...
    let tags: id3.Tags = {
      title: 'NewsByte for ' + data.airdate,
      subtitle: subtitle ? subtitle[0] : '',
      partOfSet: 'NB',
      album: 'Squarewave Symphony NewsByte',
      artist: 'KewlioMZX',
      comment: {
        language: 'eng',
      ...
      switch (item.type) {
        ...
        case DataType.Music:
          ...
          if (altData.image) {
            ...
            else chap.tags.image = altData.image = `${BASE_DATAPATH}Mus/NB/${data.shortAirdate}/${altData.image}`;
          }
  ...
  GenerateEndCredits: async (data: ShowData, { simpleSegList, segMusic }: GenerateCompanionOptions): Promise<string> => {
    ...
    retval.push(format(credits.start, data.airdate));
    retval.push(credits.notw);
    retval.push(credits.segmusic);
  ...
  GenerateStreamPlaylist: async (data: ShowData, masterList: AlternateData[]): Promise<string> => {
    ...
    for (let item of masterList) {
      ...
      const createImage = ({ artist, image }: createImageOptions) => {
        ...
        if (!fs.existsSync(`${BASE_DATAPATH}Mus/$NB/`) ||
          !fs.existsSync(`${BASE_DATAPATH}Mus/$NB/${data.shortAirdate}/`))
          fs.mkdirSync(`${BASE_DATAPATH}Mus/$NB/${data.shortAirdate}`, { recursive: true });
        // TODO: extract image, resize to 100x100, save to overlay resource dir
        return `./Mus/$NB/${data.shortAirdate}/${artist}-100.png`;
      };
    ...
    while (!done) {
      ...
      switch (true) {
        ...
        case url.startsWith('lowbiasgaming.net/squaresym/?p=admin&do=upload'): {
          ...
          // Fill in keywords
          let keywordList = ['chiptune', 'News of the Weird', 'community radio'];
          if (data.keywords) {
          ...
          // Fill in post date
          let postDate = DateTime.fromFormat(data.airdate, 'DDD', { zone: 'America/Halifax' }).plus({ days: 4, hours: 17 });
        ...
        case url.startsWith('ckdu.ca/admin/radio_logger/episodes/'):
          ...
          for (const item of await page.$$('div.fields')) {
            ...
            switch (entryType) {
              case 'music': {
                // Get fields
                const [artist, title, album, language] = await item.$$('input[type=text]');
...
const ModSqSyNewsByte: React.FC = (props) => {
  ...
  const readShowDataDir = () => {
    if (!fs.existsSync('./showdata') || !fs.existsSync('./showdata/SqSyNewsByte')) {
...
export default ModSqSyNewsByte;